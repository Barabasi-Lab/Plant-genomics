    ...
    if 'kegg' in org2pathways[org]:
        ...
        for pt in org2pathways[org]['kegg']:
            ...
            if pts in genPathKEGG:
                ...
                org2enriched[org][pt]['pathway']=pt
                org2enriched[org][pt]['plant'] = org
                ...
                if pt not in ep2org:
                    ep2org[pt]=set()
                ep2org[pt].add(org)
        # create an org dataframe and correct for bonferroni, then append to the dataframe for all organisms
        ldf = pd.DataFrame.from_dict(org2enriched[org], orient='index')

        p_adjusted = multipletests(ldf['pval'], method='bonferroni')
        #print(ldf.pval.to_string(index=False))
        ldf['bonferroni'] = p_adjusted[1]



        # append to larger dataframe
        gen_df = gen_df.append(ldf)

        # get the enriched set
        endf = ldf[ldf['bonferroni'] <= 0.05]

        if org not in org2numenriched:
            org2numenriched[org] = set()
        org2numenriched[org] = org2numenriched[org] | set(endf.index)
    # pmn part
    ...
    for path in totpaths:
        ...
        if path not in org2enriched[org] and (path in genPathPMN or path in genPathKEGG):
            ...
            if path in genPathKEGG:
                ...
                org2enriched[org][path]['bonferroni']=path_df['bonferroni']
...
'''#try bonferoni --- edit- already corrected the p value per plant
...
sgprim=len(stprimu)/len(stpathcounts)
sgseco=len(stsecu)/len(stpathcounts)
...
#plt.xticks(catlist)
plt.tight_layout()
plt.savefig('fig_S2.svg')
plt.savefig('fig_S2.tiff')
plt.show()
...
#plt.xticks(catlist)
plt.tight_layout()
plt.savefig('fig_4C.svg')
plt.savefig('fig_4C.tiff')
plt.show()
...
#plt.xticks(catlist)
plt.tight_layout()
plt.savefig('fig_4D.svg')
plt.savefig('fig_4D.tiff')
plt.show()
...
plt.tight_layout()
plt.savefig('fig_4Bi.svg')
plt.savefig('fig_4Bi.tiff')
plt.show()
#distribution
plt.figure(figsize=(18, 6))
sns.set(font_scale=2.5)
...
plt.tight_layout()
plt.savefig("fig_S3.svg")
plt.savefig("fig_S3.tiff")
plt.show()
...
plt.tight_layout()
plt.savefig("fig_4A.svg")
plt.savefig("fig_4A.tiff")
plt.show()
